...
// Init list
var shipmentsList = new List("shipmentsList", options).on(
  "updated",
  function (list) {
...
xhttp.onload = function () {
  ...
  shipmentsList.remove(
    "id",
    `<a href="javascript:void(0);" class="fw-medium link-primary">#TBSC2830</a>`
  );
};
...
function isStatus(val) {
  switch (val) {
    ...
    case "Returns":
      return (
        '<span class="badge bg-primary-subtle text-primary ">' + val + "</span>"
      );
    case "Pending":
      return (
        '<span class="badge bg-warning-subtle text-warning ">' + val + "</span>"
    ...
    Array.from(editBtns).forEach(function (btn) {
      btn.addEventListener("click", function (e) {
        ...
        Array.from(itemValues).forEach(function (x) {
          ...
          var ship_no = new DOMParser().parseFromString(
            x._values.shipment_no,
            "text/html"
          );
    ...
    if (
      ...
      shipmentsList.add({
        id:
          ...
          "</a>",
        shipment_no:
          '<a href="javascript:void(0);" class="text-reset">#TBSN15414524986</a>',
