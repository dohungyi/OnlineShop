    ...
    columns: [
      {
        ...
        data: function (row) {
          return gridjs.html(
            '<div class="fw-medium">#TBSC' + row.id + "</div>"
          );
        },
      },
      {
        ...
        data: function (row) {
          return gridjs.html(
            '<ul class="hstack gap-2 list-unstyled mb-0">\
                    <li>\
                        <a href="#" class="badge bg-success-subtle text-success " onClick="editCategoryList(' +
              row.id +
              ')">Edit</a>\
                    </li>\
                    <li>\
                        <a href="#removeItemModal" data-bs-toggle="modal" class="badge bg-danger-subtle text-danger " onClick="removeItem(' +
              row.id +
              ')">Delete</a>\
                    </li>\
                </ul>'
...
searchResultList.addEventListener("keyup", function () {
  ...
  function filterItems(arr, query) {
    return arr.filter(function (el) {
      return (
        el.subcategory.toLowerCase().indexOf(query.toLowerCase()) !== -1 ||
        el.category.toLowerCase().indexOf(query.toLowerCase()) !== -1 ||
        el.createby.toLowerCase().indexOf(query.toLowerCase()) !== -1
      );
    });
  }
...
function findNextId() {
  ...
  var lastElementId = fetchIdFromObj(
      subCategoriesData[subCategoriesData.length - 1]
    ),
...
function editCategoryList(elem) {
  ...
  subCategoriesData = subCategoriesData.map(function (item) {
    if (item.id == getEditid) {
      editList = true;
      document.getElementById("addCategoryLabel").innerHTML =
        "Edit Sub Categories";
