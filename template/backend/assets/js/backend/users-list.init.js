...
var options = {
  valueNames: ["id", "user_name", "email_id", "date", "status"],
  page: perPage,
  pagination: true,
  plugins: [
    ListPagination({
      ...
      btn.addEventListener("click", function (e) {
        ...
        Array.from(itemValues).forEach(function (x) {
          ...
          if (selectedid == itemId) {
            ...
            var userName = new DOMParser().parseFromString(
              x._values.user_name,
              "text/html"
            );

            var userImgVal =
              userName.body.querySelector(".user-profile-img").src;
            usersImg.src = userImgVal;

            var userNameVal =
              userName.body.querySelector(".user_name").innerHTML;
    ...
    if (
      ...
      usersList.add({
        ...
        user_name:
          ...
          usersImg.src +
          '" alt="" class="avatar-xs rounded-circle user-profile-img"></div>\
                <div class="flex-grow-1 ms-2 user_name">' +
          userNameField.value +
          "</div>\
                </div>",
...
function clearFields() {
  ...
  document.getElementById("users-img-field").src =
...
// brandLogo image
document
  .querySelector("#users-image-input")
